...
def updatedict(f):
    ...
    for line in f: #for loop to read each line in sales records file
        splitline = writefile(line) #call writefile function which will write each line to its respective output city file, and store 
...
#writefile takes in the variable line, which is each line form the sales record file. This line is then broken down into a list using .split() method.
...
# to that file. Afterwards we return the list as it will be use for dictionary creation later.    
def writefile(line):
